...
for run = 1:runs
    ...
    %% Main Loop Watkins Q(l=0) Learning
    ...
    for e = 1:maxEpisode
        ...
        for s = 1:maxStep
            ...
            delta = reward + gamma*Agent.Q(nx, ny, next_action_star) - Agent.Q(x, y, action);
            ...
            for sx = 1:xl
                for sy = 1:yl
                    for act = 1:4
                        Agent.Q(sx, sy, act) = Agent.Q(sx, sy, act) + alpha*delta*Agent.Z(sx, sy, act);
                        if Agent.Z(sx, sy, act) ~= 0
                            if next_action == next_action_star
                                Agent.Z(sx, sy, act) = gamma * lambda * Agent.Z(sx, sy, act);
                            else
                                Agent.Z(sx, sy, act) = 0;
                            end
                        end
    ...
    save(['.\Params\LZ_TE_R', num2str(run), '.mat'],'test_rewards');
    save(['.\Params\LZ_TR_R', num2str(run), '.mat'],'mean_rewards');
    save(['.\Params\LZ_TE_TS_R', num2str(run), '.mat'],'test_totalsteps');
    disp(['Number: ', num2str(run)]);
end
