...
for alpha_run = 1:runs
    for gamma_runs = 1:runs
        ...
        %% Main Loop Watkins Q(l=1) Learning
        ...
        for e = 1:maxEpisode
                        ...
                        for act = 1:4
                            ...
                            if Agent.Z(sx, sy, act) ~= 0
                                if next_action == next_action_star
                                    Agent.Z(sx, sy, act) = gamma(gamma_runs) * lambda * Agent.Z(sx, sy, act);
                                else
                                    Agent.Z(sx, sy, act) = 0;
                                end
                            end
            ...
            %disp(['Iteration: ' num2str(e) ' - Mean Rewards: ' num2str(mean_rewards(e)) ' epsilon: ' num2str(epsilon)]);
        end
        % Save Results
        save(['E:\MS Term 4\Paper RL\Final Model\Static\Variant Params LO\LO_TE_R', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda), '.mat'],'test_rewards');
        save(['E:\MS Term 4\Paper RL\Final Model\Static\Variant Params LO\LO_TR_R', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda), '.mat'],'mean_rewards');
        save(['E:\MS Term 4\Paper RL\Final Model\Static\Variant Params LO\LO_TE_TS_R', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda), '.mat'],'test_totalsteps');
        disp(['Number: ', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda)]);
    end
end
%% Results
