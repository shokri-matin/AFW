...
for alpha_run = 1:runs
    for gamma_runs = 1:runs
        for lambda_runs = 1:runs
            ...
            % Save Results
            % Save Results
            save(['E:\MS Term 4\Paper RL\Final Model\Static\Variant Params\W_TE_R', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda(lambda_runs)), '.mat'],'test_rewards');
            save(['E:\MS Term 4\Paper RL\Final Model\Static\Variant Params\W_TR_R', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda(lambda_runs)), '.mat'],'mean_rewards');
            save(['E:\MS Term 4\Paper RL\Final Model\Static\Variant Params\W_TE_TS_R', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda(lambda_runs)), '.mat'],'test_totalsteps');
            disp(['Number: ', num2str(run),'_',num2str(alpha(alpha_run)),'_',num2str(gamma(gamma_runs)),'_',num2str(lambda(lambda_runs))]);
        end
    end
end
